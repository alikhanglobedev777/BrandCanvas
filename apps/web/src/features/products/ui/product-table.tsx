...
import { formatProductStatus, getProductStatusTone, getStockStatusTone } from "../lib/product-status";

export function ProductTable({ products }: { products: ProductResponseDto[] }) {
  if (products.length === 0) {
    return (
      <Paper variant="outlined">
        <EmptyState title="No products found" description="Create your first product or adjust the current filters." />
      </Paper>
    );
  }
        ...
        <TableBody>
          {products.map((product) => (
            <TableRow key={product.id} hover>
              ...
              <TableCell>
                <StatusChip label={formatProductStatus(product.status)} tone={getProductStatusTone(product.status)} />
              </TableCell>
