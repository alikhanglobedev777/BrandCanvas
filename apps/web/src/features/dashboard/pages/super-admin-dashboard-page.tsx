...
export function SuperAdminDashboardPage() {
  ...
  const summary = useStoreGetSummary({
    query: { enabled: account.data?.platformRole === "super_admin" },
  });
    ...
    return (
      <>
        <PageHeader
          ...
          description="Your product catalog, inventory controls, store settings, and branding workspace are all available from the seller navigation."
          actions={
            <Chip
              label={account.data?.storeStatus ?? "No store"}
              color="primary"
              variant="outlined"
            />
          }
        />
        <Alert severity="info">
          Your seller account is connected to store ID:{" "}
          {account.data?.storeId ?? "Not assigned"}.
        </Alert>
      </>
    );
  }
  ...
  const cards = [
    ...
    { label: "Active stores", value: summary.data.active, shortLabel: "AS" },
    {
      ...
      value: summary.data.inactive,
      shortLabel: "IS",
    },
    {
      label: "Suspended stores",
      value: summary.data.suspended,
      shortLabel: "SS",
    },
  ];
  ...
  return (
    <>
      <PageHeader
        ...
        actions={
          <Chip
            label={`${summary.data.pending} pending`}
            color="warning"
            variant="outlined"
          />
        }
